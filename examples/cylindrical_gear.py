...
def cylindrical_gear(m, z, alpha, b, helix_angle = None):
    ...
    def point_on_circle(radius, angle):
        return cq.Vector(radius*cos(angle), radius*sin(angle), 0)

    def midpoint_on_arc(radius, start_pt, end_pt):
        start_angle = atan2(start_pt.y, start_pt.x)
        end_angle = atan2(end_pt.y, end_pt.x)
        delta = (end_angle - start_angle + pi) % (2*pi) - pi
        return point_on_circle(radius, start_angle + delta/2)
    ...
    def create_tooth_profile(m, z, alpha):
        ...
        def tooth_profile(loc):
            ...
            right = cq.Workplane("XY").parametricCurve(involute(r_b), stop = STOP, makeWire=False).val()
            left = (
                cq.Workplane("XY")
                .transformed(rotate=(0,0,180/z))
            ...
            if r_b < r_f:                
                ...
                left_base = left.startPoint()
                left_tip = left.endPoint()
                right_base = right.startPoint()
                right_tip = right.endPoint()

                left_root = point_on_circle(r_f, atan2(left_base.y, left_base.x))
                right_root = point_on_circle(r_f, atan2(right_base.y, right_base.x))

                left_side = cq.Edge.makeLine(left_root, left_base)
                right_side = cq.Edge.makeLine(right_base, right_root)
                top_arc = cq.Edge.makeThreePointArc(
                    left_tip,
                    midpoint_on_arc(r_a, left_tip, right_tip),
                    right_tip,
                )
                root_arc = cq.Edge.makeThreePointArc(
                    right_root,
                    midpoint_on_arc(r_f, right_root, left_root),
                    left_root,
                )

                wire = cq.Wire.assembleEdges(
                    [left, left_side, root_arc, right_side, right, top_arc]
                )
            return wire.moved(loc)  #took the trick from slot2D function, eachpoint feeds via the previously created polararray
